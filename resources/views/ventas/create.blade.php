            ...
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body">
                    @can('create', App\Models\Venta::class)
                        <form action="{{ route('ventas.store') }}" method="POST" novalidate class="js-validate-form">
                            @csrf
                                ...
                                <div class="mb-3">
                                    <label for="fecha_venta" class="form-label fw-bold">Fecha de Venta <span class="text-danger">*</span></label>
                                     <input type="date"
                                         ...
                                         value="{{ old('fecha_venta', now()->format('Y-m-d')) }}"
                                         required
                                         aria-required="true"
                    ...
                    @else
                        <x-alert type="warning">
                            No tienes permiso para crear ventas. <a href="{{ route('ventas.index') }}" class="alert-link">Volver</a>
                        </x-alert>
                    @endcan
                </div>
            </div>
